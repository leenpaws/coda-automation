...
def single_transaction(use_new_account=False, fee=4, amount=None):
    ...
    codalib.coda_send_transaction(big_key,
                                  receiver,
                                  amount=amount,
                                  fee=fee,
                                  port=clientport)

    previous_balance = codalib.coda_get_balance(key=receiver[0],
                                                port=clientport)
    expected_balance = previous_balance + amount

    logger.info('Initial Balance: %s - Expected Balance: %s' %
    ...
    while True:
        ...
        if last_block < current_block:
            logger.info('Current Block: %s -- Last Block: %s' %
                        (current_block, last_block))
            last_block = current_block

        if current_block > initial_block + max_blocks:
            logger.info('ERROR: Transaction failed. %s blocks passed' %
                        max_blocks)
            #sys.exit(1)
            break
